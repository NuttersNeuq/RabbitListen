...
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_createEngine(
...
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_releaseEngine(
		JNIEnv* env, jclass clazz) {
...
JNIEXPORT jboolean Java_com_lfl_opensl_audio_OSLESMediaPlayer_createAudioPlayer(
...
/*
	result = (*uriPlaybackPitch)->GetPitchCapabilities(uriPlaybackPitch,
			&playbackMinPitch, &playbackMaxPitch);
	assert(SL_RESULT_SUCCESS == result);*/
/*
	SLpermille minRate, maxRate, stepSize, rate = 1000;
	SLuint32 capa;
	(*uriPlaybackRate)->GetRateRange(uriPlaybackRate, 0, &minRate, &maxRate,
			&stepSize, &capa);
...
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_releaseAudioPlayer(
		JNIEnv* env, jclass clazz) {
	// destroy URI audio player object, and invalidate all associated interfaces
	if (uriPlayerObject != NULL) {
		(*uriPlayerObject)->Destroy(uriPlayerObject);
...
// play
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_play(JNIEnv* env,
		jclass clazz) {
	setPlayState(SL_PLAYSTATE_PLAYING);
}

// stop
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_stop(JNIEnv* env,
		jclass clazz) {
	setPlayState(SL_PLAYSTATE_STOPPED);
}

// pause
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_pause(JNIEnv* env,
		jclass clazz) {
	setPlayState(SL_PLAYSTATE_PAUSED);
}

// pause
JNIEXPORT jboolean Java_com_lfl_opensl_audio_OSLESMediaPlayer_isPlaying(
		JNIEnv* env, jclass clazz) {
	return (getPlayState() == SL_PLAYSTATE_PLAYING);
}

// set position
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_seekTo(JNIEnv* env,
		jclass clazz, jint position) {
	if (NULL != uriPlayerPlay) {
...
// get duration
JNIEXPORT jint Java_com_lfl_opensl_audio_OSLESMediaPlayer_getDuration(
		JNIEnv* env, jclass clazz) {
	if (NULL != uriPlayerPlay) {
...
JNIEXPORT jint Java_com_lfl_opensl_audio_OSLESMediaPlayer_getPosition(
		JNIEnv* env, jclass clazz) {
	if (NULL != uriPlayerPlay) {
...
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_setPitch(JNIEnv* env,
		jclass clazz, jint rate) {
...
JNIEXPORT void Java_com_lfl_opensl_audio_OSLESMediaPlayer_setRate(JNIEnv* env,
		jclass clazz, jint rate) {
...
JNIEXPORT jint Java_com_lfl_opensl_audio_OSLESMediaPlayer_getRate(JNIEnv* env,
		jclass clazz) {
	if (NULL != uriPlaybackRate) {
		SLresult result;
...
// create URI audio player
JNIEXPORT jboolean Java_com_lfl_opensl_audio_OSLESMediaPlayer_setLoop(
...
// create URI audio player
JNIEXPORT jboolean Java_com_lfl_opensl_audio_OSLESMediaPlayer_setNoLoop(
		JNIEnv* env, jclass clazz) {
	SLresult result;
